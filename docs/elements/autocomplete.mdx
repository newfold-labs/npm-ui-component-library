import { Root as NewfoldRoot, Autocomplete } from "@newfold/ui-component-library";
import PropsTable from '@site/src/components/PropsTable';
import { AutocompleteDemo, AutocompleteValidationVariants, autocompleteProps } from '@site/docs/partials/autocomplete';

# Autocomplete
A simple autocomplete select component.

<NewfoldRoot>
    <AutocompleteDemo />
</NewfoldRoot>

---
### Usage
```jsx
import { Autocomplete } from "@newfold/ui-component-library";

<Autocomplete
    id="autocomplete-input"
    onChange={handleChange}
    onQueryChange={handleQueryChange}
    placeholder="Type to autocomplete options"
    value=""
>
    <Autocomplete.Option value="1">
        Option 1
    </Autocomplete.Option>
    <Autocomplete.Option value="2">
        Option 2
    </Autocomplete.Option>
    <Autocomplete.Option value="3">
        Option 3
    </Autocomplete.Option>
</Autocomplete>
```

---
### Label
An example with a label using `label` prop.

<NewfoldRoot>
    <AutocompleteDemo label="Select an Option"/>
</NewfoldRoot>

```jsx
<Autocomplete
    id="autocomplete-input"
    onChange={handleChange}
    onQueryChange={handleQueryChange}
    placeholder="Type to autocomplete options"
    value=""
    label="Select an Option"
>
    <Autocomplete.Option value="1">
        Option 1
    </Autocomplete.Option>
    <Autocomplete.Option value="2">
        Option 2
    </Autocomplete.Option>
    <Autocomplete.Option value="3">
        Option 3
    </Autocomplete.Option>
</Autocomplete>
```

---
### Placeholder
An example with placeholder using `placeholder` prop.

<NewfoldRoot>
    <AutocompleteDemo 
        label="Select an Option"
        placeholder="A custom placeholder"
    />
</NewfoldRoot>

```jsx
<Autocomplete
    id="autocomplete-input"
    onChange={handleChange}
    onQueryChange={handleQueryChange}
    value=""
    label="Select an Option"
    placeholder="A custom placeholder"
>
    <Autocomplete.Option value="1">
        Option 1
    </Autocomplete.Option>
    <Autocomplete.Option value="2">
        Option 2
    </Autocomplete.Option>
    <Autocomplete.Option value="3">
        Option 3
    </Autocomplete.Option>
</Autocomplete>
```

---
### Selected Label
An example with default value using `selectedLabel` prop.

<NewfoldRoot>
    <AutocompleteDemo 
        label="Select an Option"
        placeholder="A custom placeholder"
        selectedLabel="Option 2"
        value="2"
    />
</NewfoldRoot>

```jsx
<Autocomplete
    id="autocomplete-input"
    onChange={handleChange}
    onQueryChange={handleQueryChange}
    value="2"
    label="Select an Option"
    placeholder="Type to autocomplete options"
    selectedLabel="Option 2"
>
    <Autocomplete.Option value="1">
        Option 1
    </Autocomplete.Option>
    <Autocomplete.Option value="2">
        Option 2
    </Autocomplete.Option>
    <Autocomplete.Option value="3">
        Option 3
    </Autocomplete.Option>
</Autocomplete>
```

---
### Nullable
Allow empty values with reset button `X` or deleting the option and clicking outside the field.

<NewfoldRoot>
    <AutocompleteDemo 
        label="Select an Option"
        placeholder="Type to autocomplete options"
        nullable={true}
    />
</NewfoldRoot>

```jsx
<Autocomplete
    id="autocomplete-input"
    onChange={handleChange}
    onQueryChange={handleQueryChange}
    value=""
    label="Select an Option"
    placeholder="Type to autocomplete options"
    selectedLabel=""
>
    <Autocomplete.Option value="1">
        Option 1
    </Autocomplete.Option>
    <Autocomplete.Option value="2">
        Option 2
    </Autocomplete.Option>
    <Autocomplete.Option value="3">
        Option 3
    </Autocomplete.Option>
</Autocomplete>
```

---
### Validation
You can use the `validation` prop to render a validation variant to the Autocomplete component.
The Autocomplete component provide four validation variants: `success`, `warning`, `info`, and `error`.
<AutocompleteValidationVariants />

---
### onChange
The `onChange` prop is a function that will be called when the value of the Autocomplete component changes.
The function will receive the new value of the Autocomplete component as a parameter.

```jsx
<Autocomplete
    id="autocomplete-input"
    onChange={new_val => console.log(new_val)}
    onQueryChange={handleQueryChange}
>
    <Autocomplete.Option value="1">
        Option 1
    </Autocomplete.Option>
    <Autocomplete.Option value="2">
        Option 2
    </Autocomplete.Option>
    <Autocomplete.Option value="3">
        Option 3
    </Autocomplete.Option>
</Autocomplete>
```

---
### onQueryChange
The `onQueryChange` prop is a function that will be called when the query of the Autocomplete component changes.
The function will receive the new query event of the Autocomplete component as a parameter.

```jsx
<Autocomplete
    id="autocomplete-input"
    onChange={(new_val) => console.log(new_val)}
    onQueryChange={(event) => console.log(event.target.value)}
>
    <Autocomplete.Option value="1">
        Option 1
    </Autocomplete.Option>
    <Autocomplete.Option value="2">
        Option 2
    </Autocomplete.Option>
    <Autocomplete.Option value="3">
        Option 3
    </Autocomplete.Option>
</Autocomplete>
```

---
### Props
<PropsTable.Table docs={autocompleteProps} />
