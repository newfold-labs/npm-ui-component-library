...
`.nfd-root` CSS class for scoping our CSS in opinionated environments. You can use the
`RootContext` by using the `useRootContext` hook exported from `@newfold/ui-component-library/hooks`.
