...
export const Tag = ( { tag, index, disabled = false, onRemoveTag, screenReaderRemoveTag, ...props } ) => {
	...
	return (
		...
		<Badge
			onKeyDown={ handleKeyDown } { ...props } variant="plain" className="nfd-tag-input__tag"
		>
			<span className="nfd-mb-px">{ tag }</span>
			<button onClick={ handleClick } className="nfd-tag-input__remove-tag">
				<span className="nfd-sr-only">{ screenReaderRemoveTag }</span>
				<XIcon className="nfd-h-3 nfd-w-3" />
			</button>
		</Badge>
	);
};
...
const TagInput = forwardRef( ( {
	...
	return (
		<div className={ classNames( "nfd-tag-input", disabled && "nfd-tag-input--disabled", className ) }>
