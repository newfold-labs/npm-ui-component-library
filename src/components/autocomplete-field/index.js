...
const AutocompleteField = forwardRef( ( {
	...
	return (
		<div className={ classNames( "nfd-autocomplete-field", className ) }>
			<Autocomplete
				ref={ ref }
				id={ id }
				label={ label }
				labelProps={ {
					as: "label",
					className: "nfd-label nfd-autocomplete-field__label",
				} }
				validation={ validation }
				className="nfd-autocomplete-field__select"
				buttonProps={ { "aria-describedby": describedBy } }
				{ ...props }
			/>
			{ validation?.message && (
				<ValidationMessage variant={ validation?.variant } id={ ids.validation } className="nfd-autocomplete-field__validation">{ validation.message }</ValidationMessage>
			) }
			{ description && <div id={ ids.description } className="nfd-autocomplete-field__description">{ description }</div> }
		</div>
	);
} );
