...
const TagField = forwardRef( ( {
	...
	return (
		<div className={ classNames( "nfd-tag-field", disabled && "nfd-tag-field--disabled", className ) }>
			<div className="nfd-flex nfd-items-center nfd-mb-2">
				<Label className="nfd-tag-field__label" htmlFor={ id } label={ label } />
				{ labelSuffix }
			</div>
			<ValidationInput
				as={ TagInput }
				...
				id={ id }
				disabled={ disabled }
				className="nfd-tag-field__input"
				aria-describedby={ describedBy }
				validation={ validation }
				{ ...props }
			/>
			{ validation?.message && (
				<ValidationMessage variant={ validation?.variant } id={ ids.validation } className="nfd-tag-field__validation">
					{ validation.message }
				</ValidationMessage>
			) }
			{ description && <p id={ ids.description } className="nfd-tag-field__description">{ description }</p> }
		</div>
	);
} );
