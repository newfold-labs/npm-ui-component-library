...
const SelectField = forwardRef( ( {
	...
	return (
		<div className={ classNames( "nfd-select-field", disabled && "nfd-select-field--disabled", className ) }>
			<Select
				ref={ ref }
				id={ id }
				label={ label }
				labelProps={ {
					as: "label",
					className: "nfd-label nfd-select-field__label",
				} }
				disabled={ disabled }
				validation={ validation }
				className="nfd-select-field__select"
				buttonProps={ { "aria-describedby": describedBy } }
				{ ...props }
			/>
			{ validation?.message && (
				<ValidationMessage variant={ validation?.variant } id={ ids.validation } className="nfd-select-field__validation">
					{ validation.message }
				</ValidationMessage>
			) }
			{ description && <div id={ ids.description } className="nfd-select-field__description">{ description }</div> }
		</div>
	);
} );
